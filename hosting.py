...
STREAM_URL = "http://192.168.141.118:8000/video_feed"
...
spinner.text = "Connecting to the streaming source"
spinner.start()
videoCapture = cv2.VideoCapture(STREAM_URL)
spinner.stop()

if videoCapture.isOpened():
    print("Connected to the streaming source!")
else:
    print(f"Error while connecting to {STREAM_URL}")
    exit(1)
...
def custom_face_detection(frame):
    # # Your custom face detection logic goes here
    # # Modify the frame as needed

    # # For this example, we'll just draw a rectangle around the center of the frame
    # height, width, _ = frame.shape
    # top_left = (height // 4, width // 4)
    # bottom_right = (3 * height // 4, 3 * width // 4)
    # color = (0, 255, 0)  # Green
...
with Pool(cpu_count()) as pool:
    while running:
        ...
        # Process the frame using your custom face detection logic
        frame = custom_face_detection(frame)
